 ...
 * 2) LinkedBlockingQueue - The LinkedBlockingQueue keeps the elements internally in a linked structure (linked nodes).
 ...
 * 3) LinkedTransferQueue - В отличие от реализации очередей интерфейса BlockingQueue, где потоки могут быть блокированы при чтении,
 ...
 * 4) SynchronousQueue - The SynchronousQueue is a queue that can only contain a single element internally. Если у нее
 ...
 * 5) DelayQueue - очередь, в которую могут быть помещены элементы, имплементирующие интерфейс Delayed. В этой очереди
 ...
 * 6) PriorityBlockingQueue  - The PriorityBlockingQueue is an unbounded concurrent queue. It uses the same ordering rules
