...
//Spurious Wakeup - это ситуация, когда нить просыпается от wait без вызова notify. Это может произойти по разным причинам,
//например, из-за ошибок в планировщике потоков или из-за аппаратных сбоев.
//Поэтому рекомендуется использовать wait внутри цикла, который проверяет условие, чтобы убедиться, что нить проснулась по правильной причине.

// ❌ ПЛОХО
//synchronized (lock) {
//		if (queue.isEmpty()) { // if вместо while
//		lock.wait();
//    }
//			queue.poll(); // Проблема: может проснуться, а очередь всё ещё пуста!
//}
//
//// ✅ ХОРОШО
//synchronized (lock) {
//		while (queue.isEmpty()) { // while - перепроверяем условие
//		lock.wait();
//    }
//			queue.poll(); // Безопасно
//}



// TODO NOTIFY(механизм работы):
// notify используется для пробуждения нитей, которые уснули с помощью метода wait. Спящие нити не позволят программе завершиться
// Как только текущая нить выйдет из блока synchronized, одна из проснувшихся нитей захватит мютекс и продолжит свою работу.
// Когда она освободит мютекс, другая проснувшаяся нить захватит мютекс и т.д. Лучше использовать notifyAll() метод, чтобы
// не проснувшийся поток не остался заблокированым навсегда
