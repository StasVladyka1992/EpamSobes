...
public class Main {
	public static void main(String[] args) {
		...
		main.takeWhileExample();
		main.dropWhile();
	}

	private void takeWhileExample() {
		List<Integer> result = Stream.iterate(1, s -> ++s).takeWhile(s -> s < 10).collect(Collectors.toList());
        System.out.println(result);
	}

	private void dropWhile(){
	    List<Integer> result = Arrays.asList(1,2,3,4,5).stream().dropWhile(s -> s<3).collect(Collectors.toList());
        System.out.println(result);
    }
