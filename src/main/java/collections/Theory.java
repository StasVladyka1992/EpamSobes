...
Иерархия коллекций - https://habr.com/ru/articles/237043/
...
Внутри ArrayList (и большинства коллекций) есть счётчик modCount — количество структурных модификаций (добавление, удаление, очистка...).
Когда вы создаёте итератор (а for-each неявно его создаёт), итератор запоминает текущее значение modCount в своём поле expectedModCount.

При каждом вызове next() итератор проверяет:

if (modCount != expectedModCount) throw new ConcurrentModificationException();
list.remove() увеличивает modCount, но не обновляет expectedModCount у уже созданного итератора → проверка проваливается → исключение.

Как правильно удалять элементы:
- Способ 1: Явный Iterator с iterator.remove()
