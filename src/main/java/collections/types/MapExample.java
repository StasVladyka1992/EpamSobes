...
// - изначальное кол-во bucket - 16. Load factor - 0,75. Почему 16, потому что это кратно 2
// и связано с определением бакета для элементов.
// При достижении количества элементов равное capacity * loadFactor размер увеличивается в два раза.
// элементы перераспределяются по новым корзинам. null ключ возможен (только 1)
...
// Коллизия — это ситуация, когда два разных ключа получили одинаковый индекс бакета после вычисления (n - 1) & hash.

// При добавлении элемента его бакет вычисляется по хэшу ключа: index = (n - 1) & hash.
// int hash = key.hashCode();
// int index = hash % array.length;
// Почему не hash % n (остаток от деления)?
// Потому что & работает гораздо быстрее на уровне процессора, чем %.
// Но это работает только если n — степень двойки.
// (n - 1) & hash — это быстрая замена операции взятия остатка hash % n, работающая только когда n — степень двойки.
// Результат — младшие биты хэша, которые и дают индекс бакета от 0 до n-1.
