 ...
 * 3) LinkedBlockingQueue - The LinkedBlockingQueue keeps the elements internally in a linked structure (linked nodes).
 ...
 * 4) PriorityBlockingQueue  - The PriorityBlockingQueue is an unbounded concurrent queue. It uses the same ordering rules
 * 	 as the java.util.PriorityQueue class. You cannot insert null into this queue.
 * 5) SynchronousQueue - The SynchronousQueue is a queue that can only contain a single element internally. A thread inseting an element
 ...
 * 6) LinkedTransferQueue - В отличие от реализации очередей интерфейса BlockingQueue, где потоки могут быть блокированы при чтении,
